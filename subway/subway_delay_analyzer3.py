...
class SubwayDelayAnalyzer:
    ...
    def filter_by_line(self, timetable, realtime, target_line, updnLine=None):
        """특정 노선 데이터 필터링"""
        # 노선 번호 매핑 (1호선 -> 1001)
        ...
        # 시간표 필터링
        timetable_filtered = timetable[timetable['호선'] == (subway_id - 1000)].copy()
        
        # 실시간 데이터 필터링
        realtime_filtered = realtime[realtime['subwayId'] == subway_id].copy()

        if updnLine is not None:
            updn_mapping_realtime1 = {
            "상행": "상행", "내선": "상행",
            "하행": "하행", "외선": "하행"
            }
            updn_mapping_realtime2 = {
            "상행": "내선", "내선": "내선",
            "하행": "외선", "외선": "외선"
            }
            updn_mapping_timetable = {
                "상행": "UP", "내선": "UP",
                "하행": "DOWN", "외선": "DOWN"
            }
            
            if updnLine not in updn_mapping_realtime1:
                raise ValueError(f"지원하지 않는 방향(updnLine)입니다: {updnLine}")
            
            rt_dir1 = updn_mapping_realtime1[updnLine]
            rt_dir2 = updn_mapping_realtime2[updnLine]
            tt_dir = updn_mapping_timetable[updnLine]
            
            # realtime에서 상행/하행 필터
            if 'updnLine' in realtime_filtered.columns:
                realtime_filtered = realtime_filtered[(realtime_filtered['updnLine'] == rt_dir1) | (realtime_filtered['updnLine'] == rt_dir2)]
            
            # timetable에서 UP/DOWN 필터
            if '방향' in timetable_filtered.columns:
                timetable_filtered = timetable_filtered[timetable_filtered['방향'] == tt_dir]
        
        return timetable_filtered, realtime_filtered
    
    def match_train_data(self, timetable, realtime, time_tolerance=300):
        """열차 데이터 매칭 (시간표 vs 실시간)"""
        matched_data = []
        
        # 실시간 데이터를 열차별로 그룹화
        train_groups = realtime.groupby('btrainNo')
        
        for train_no, train_data in train_groups:
            train_data = train_data.sort_values('recptnDt')
            
            # 각 역에 대해 시간표와 매칭
            for _, real_record in train_data.iterrows():
                station_name = real_record['statnNm']
                real_time = real_record['recptnDt']
                
                # 시간표에서 해당 역과 비슷한 시간대의 기록 찾기
                station_timetable = timetable[
                    (timetable['역사명'] == station_name) &
                    (timetable['방향'] == ('DOWN' if real_record['updnLine'] == '하행' else 'UP'))
                ]
                
                if len(station_timetable) == 0:
                    continue
                
                # 가장 가까운 시간의 열차 찾기
                time_diffs = []
                for _, schedule_record in station_timetable.iterrows():
                    schedule_time = schedule_record['열차도착시간']
                    
                    # 날짜는 무시하고 시간만 비교
                    real_time_of_day = real_time.time()
                    schedule_time_of_day = schedule_time.time()
                    
                    # 시간 차이 계산 (초 단위)
                    real_seconds = real_time_of_day.hour * 3600 + real_time_of_day.minute * 60 + real_time_of_day.second
                    schedule_seconds = schedule_time_of_day.hour * 3600 + schedule_time_of_day.minute * 60 + schedule_time_of_day.second
                    
                    time_diff = abs(real_seconds - schedule_seconds)
                    time_diffs.append((time_diff, schedule_record))
                
                if time_diffs:
                    # 가장 가까운 시간의 기록 선택
                    min_diff, best_match = min(time_diffs, key=lambda x: x[0])
                    
                    if min_diff <= time_tolerance:  # 5분 이내
                        delay_seconds = (real_time.time().hour * 3600 + real_time.time().minute * 60 + real_time.time().second) - \
                                      (best_match['열차도착시간'].time().hour * 3600 + best_match['열차도착시간'].time().minute * 60 + best_match['열차도착시간'].time().second)
                        
                        matched_data.append({
                            'train_no': train_no,
                            'station_name': station_name,
                            'direction': real_record['updnLine'],
                            'scheduled_time': best_match['열차도착시간'],
                            'actual_time': real_time,
                            'delay_seconds': delay_seconds,
                            'train_code': best_match['열차코드']
                        })
    ...
    def analyze_line_delays(self, timetable_path, realtime_path, target_line, updnLine, time_tolerance=300):
        """전체 분석 파이프라인 실행"""
        print(f"=== {target_line} 지연 분석 시작 ===")
        
        # 1. 데이터 로드
        print("1. 데이터 로딩...")
        timetable = self.load_timetable(timetable_path)
        realtime = self.load_realtime_data(realtime_path)
        
        # 2. 노선별 필터링
        print("2. 노선 데이터 필터링...")
        timetable_filtered, realtime_filtered = self.filter_by_line(timetable, realtime, target_line, updnLine)
        print(f"   - 시간표 데이터: {len(timetable_filtered)}개 레코드")
        print(f"   - 실시간 데이터: {len(realtime_filtered)}개 레코드")
        
        # 3. 데이터 매칭
        print("3. 시간표-실시간 데이터 매칭...")
        matched_data = self.match_train_data(timetable_filtered, realtime_filtered, time_tolerance)
    ...
    def create_spatiotemporal_heatmap(self, train_timeseries, start_station, end_station, time_interval_minutes=10):
        """1. 시공간 누적 히트맵"""
        filtered_timeseries, target_stations = self.filter_by_station_range(
            train_timeseries, start_station, end_station
        )
    ...
    def comprehensive_accumulation_analysis(self, train_timeseries, start_station, end_station, save_results=True):
        ...
        # 1. 시공간 히트맵
        print("1. 시공간 누적 히트맵 생성...")
        fig1, heatmap_data = self.create_spatiotemporal_heatmap(train_timeseries, start_station, end_station)
...
if __name__ == "__main__":
    ...
    date = '0529'
    time_of_day = 'evening'
    timetable_path = f'서울교통공사_서울 도시철도 열차운행시각표_20250430_{time_of_day}_clean.csv'
    ...
    if train_timeseries:
        ...
        results, figures = analyzer.comprehensive_accumulation_analysis(train_timeseries, '노량진', '신설동')
