...
class SubwayDelayAnalyzer:
    ...
    def match_train_data(self, asof_delay):
        ...
        asof_delay = asof_delay.rename(columns={
            '호선': 'train_no', '방향': 'direction', '역사명': 'station_name',
            '급행코드': 'express_code', '예정시간': 'scheduled_time',
            '실제시간': 'actual_time', 'delay_seconds': 'delay_seconds', 'real_train': 'train_code'
        })
    ...
    def generate_delay_timeseries(self, matched_data):
        ...
        # 열차별로 그룹화
        train_groups = matched_data.groupby('train_code')
    ...
    def analyze_line_delays(self, delay_path, target_line, updnLine, express_code):
        """전체 분석 파이프라인 실행"""
        ...
        matched_data = self.match_train_data(delay_filtered)
